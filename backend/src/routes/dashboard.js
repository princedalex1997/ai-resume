...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const allAnalysis = await Analysis.find({ userId })
      .select(
    ...
    const keywordsSpark = allAnalysis
      .slice(-10)
      .map((a) => ({ v: (a?.keywordsPresent || []).length }));

    const issuesSpark = allAnalysis
      .slice(-10)
    ...
    const [recentVersions, recentAnalysis] = await Promise.all([
    ...
    //   (a, b) => new Date(b.at) - new Date(a.at).slice(0, 8),
    // );
